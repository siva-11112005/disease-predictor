class AdvancedSymptomPredictor {
    ...
    async initialize() {
        ...
        // Validate data
        if (!this.diseaseDatabase || this.diseaseDatabase.length === 0) {
            throw new Error('Disease database failed to load');
        }
        
        if (!this.symptomDatabase || this.symptomDatabase.length === 0) {
            throw new Error('Symptom database failed to load');
        }
    ...
    async predict(selectedSymptoms, patientData = {}) {
        ...
        // Match symptoms to diseases with safety checks
        for (const disease of this.diseaseDatabase) {
            // Safety check
            if (!disease || !disease.symptoms || !Array.isArray(disease.symptoms)) {
                console.warn('Skipping invalid disease:', disease);
                continue;
            }
            ...
            if (matchPercentage > 0) {
                ...
                if (patientData.age && disease.ageRisk && disease.ageRisk.high) {
                    if (patientData.age >= disease.ageRisk.high.min && 
                ...
                if (patientData.gender && disease.genderRisk && disease.genderRisk[patientData.gender]) {
                ...
                predictions.push({
                    ...
                    recommendations: disease.recommendations || ['Consult a healthcare professional'],
                    specialization: disease.specialization || 'General Medicine',
    ...
    loadDiseaseDatabase() {
        return [
            ...
            },
            {
                ...
                symptoms: ['sneezing', 'runny_nose', 'nasal_congestion', 'itchy_eyes', 
                          'watery_eyes', 'itchy_throat'],
                severity: 'mild',
                description: 'Allergic response affecting nose and eyes',
    ...
    loadSymptomDatabase() {
        return [
            ...
            { id: 'recurring_infections', label: 'Recurring Infections', category: 'immune', severity: 'moderate' },
            ...
            { id: 'sneezing', label: 'Sneezing', category: 'respiratory', severity: 'mild' },
            { id: 'runny_nose', label: 'Runny Nose', category: 'respiratory', severity: 'mild' },
            { id: 'itchy_eyes', label: 'Itchy Eyes', category: 'visual', severity: 'mild' },
            { id: 'watery_eyes', label: 'Watery Eyes', category: 'visual', severity: 'mild' },
            { id: 'itchy_throat', label: 'Itchy Throat', category: 'respiratory', severity: 'mild' }
        ];
    }
    ...
    getStatistics() {
        return {
            ...
            avgSymptomsPerDisease: Math.round(
                this.diseaseDatabase.reduce((sum, d) => sum + (d.symptoms ? d.symptoms.length : 0), 0) / 
